...
### 7、如果缓存数据在导出导入之间过期了，您又怎么处理这些数据呢？
...
### 12、Memcached服务特点及工作原理是什么？
### 13、如何实现集群中的session共享存储？
### 14、Memcached的多线程是什么？如何使用它们？
...
### 20、Memcached和服务器的local cache（比如PHP的APC、mmap文件等）相比，有什么优缺点？
### 21、Memcached和MySQL的query
### 22、如何将Memcached中item批量导入导出？
### 23、什么是二进制协议，我该关注吗？
